...
export default function MenuDesktop({ user, profileImage, profileImageSmall, navigate }) {
  return (
    <>
      <ul className="hidden md:flex space-x-8 text-xl">
        ...
        <li>
          <button onClick={() => navigate("/bancoitens")} className="relative group text-22">
        ...
        <li>
          <button onClick={() => navigate("/cadastroitem")} className="relative group text-22">
      ...
      <div className="hidden md:flex items-center space-x-4">
        {!user ? (
          <>
            <button onClick={() => navigate("/login")} className="hover:underline text-lg">Entrar</button>
            <button onClick={() => navigate("/cadastro")} className="hover:underline text-lg">Cadastrar</button>
          </>
        ) : (
          <UserAvatar profileImage={profileImage} profileImageSmall={profileImageSmall} size="md" onClick={() => navigate("/perfil")} />
        )}
      </div>
    </>
  );
